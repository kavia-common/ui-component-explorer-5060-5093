...
/**
 * PUBLIC_INTERFACE
 * getGroupPrefs - get persisted map of group-level preferences (object keyed by groupSlug).
 * Example shape:
 * {
 *   "base-components": { showAll: false, filter: "", alphaIndex: "" }
 * }
 */
export function getGroupPrefs() {
  try {
    if (!isBrowser()) return {};
    const raw = window.localStorage.getItem('sidebar.group.prefs.v1');
    if (!raw) return {};
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === 'object' ? parsed : {};
  } catch {
...
/**
 * PUBLIC_INTERFACE
 * setGroupPrefs - persist group-level preferences map.
 * @param {Record<string, any>} map
 */
export function setGroupPrefs(map = {}) {
  try {
    if (!isBrowser()) return;
    const safe = map && typeof map === 'object' ? map : {};
    window.localStorage.setItem('sidebar.group.prefs.v1', JSON.stringify(safe));
  } catch {
    // ignore
  }
}
...
/**
 * PUBLIC_INTERFACE
 * getGroupPref - read a single group's preferences object.
 * @param {string} groupSlug
 * @returns {any}
 */
export function getGroupPref(groupSlug) {
  const all = getGroupPrefs();
  return all?.[groupSlug] || {};
}

/**
 * PUBLIC_INTERFACE
 * setGroupPref - update a single group's preferences object by merging with existing.
 * @param {string} groupSlug
 * @param {Record<string, any>} prefPatch
 */
export function setGroupPref(groupSlug, prefPatch = {}) {
  try {
    if (!isBrowser()) return;
    const all = getGroupPrefs();
    const current = all?.[groupSlug] || {};
    const next = { ...current, ...prefPatch };
    const merged = { ...all, [groupSlug]: next };
    setGroupPrefs(merged);
  } catch {
    // ignore
  }
}
